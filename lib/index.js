...
function HugoAlgolia(options) {
    ...
    HugoAlgolia.prototype.setCredentials = function () {
        const configMeta = undefined;
        fs.stat(self.pathToCredentials, function(err, stat){
            if(stat && stat.isFile()) {
                configMeta = matter.read(self.pathToCredentials);
            }
        });
        ...
        else
        {
            ...
            self.localIndexName = options.index;
        }
    };
    ...
    HugoAlgolia.prototype.setUriConfig = function () {
        ...
        const configUri = configMeta ? configMeta.data.uri : options.uri;
        const baseURL = configMeta ? configMeta.data.baseURL : options.baseurl;
        console.log("configUri", configUri, "baseURL", baseURL);
        if (configUri) {
